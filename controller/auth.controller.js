...
class AuthController {
  async registration(req, res) {
    ...
    if (candidateName) return res.status(404).json({ message: 'user with the same name already exists', type: 'name' })
    const candidateEmail = await User.findOne({ where: { email } })
    if (candidateEmail)
      return res.status(404).json({ message: 'user with the same email already exists', type: 'email' })
  ...
  async login(req, res) {
    ...
    if (!user) return res.status(404).json({ message: 'user is not found', type: 'email' })
    const comparePassword = bcrypt.compareSync(password, user.password)
    if (!comparePassword) return res.status(404).json({ message: 'incorrect password specified', type: 'password' })
